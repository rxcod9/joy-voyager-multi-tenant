...
@section('content')
    <div class="container-fluid">
        ...
        @if(config('tenant-voyager.show_dev_tips'))
    ...
    <div class="page-content settings container-fluid">
        <form action="{{ route('tenant-voyager.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
                        ...
                        <div id="{{ \Illuminate\Support\Str::slug($group) }}" class="tab-pane fade in @if($group == $active) active @endif">
                            ...
                            <div class="panel-heading">
                                <h3 class="panel-title">
                                    {{ $setting->display_name }} @if(config('tenant-voyager.show_dev_tips'))<code>setting('{{ $setting->key }}')</code>@endif
                                </h3>
                                <div class="panel-actions">
                                    <a href="{{ route('tenant-voyager.settings.move_up', $setting->id) }}">
                                        <i class="sort-icons voyager-sort-asc"></i>
                                    </a>
                                    <a href="{{ route('tenant-voyager.settings.move_down', $setting->id) }}">
                            ...
                            <div class="panel-body no-padding-left-right">
                                <div class="col-md-10 no-padding-left-right">
                                    ...
                                    @elseif($setting->type == "image" || $setting->type == "file")
                                        @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('tenant-voyager.storage.disk'))->exists($setting->value))
                                            <div class="img_settings_container">
                                                <a href="{{ route('tenant-voyager.settings.delete_value', $setting->id) }}" class="voyager-x delete_value"></a>
                                                <img src="{{ Storage::disk(config('tenant-voyager.storage.disk'))->url($setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                            </div>
                                        ...
                                        @elseif($setting->type == "file" && isset( $setting->value ))
                                            @if(json_decode($setting->value) !== null)
                                                @foreach(json_decode($setting->value) as $file)
                                                  <div class="fileType">
                                                    <a class="fileType" target="_blank" href="{{ Storage::disk(config('tenant-voyager.storage.disk'))->url($file->download_link) }}">
                                                      {{ $file->original_name }}
                                                    </a>
                                                    <a href="{{ route('tenant-voyager.settings.delete_value', $setting->id) }}" class="voyager-x delete_value"></a>
                                                 </div>
        ...
        <div class="panel" style="margin-top:10px;">
            ...
            <div class="panel-body">
                <form action="{{ route('tenant-voyager.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="setting_tab" class="setting_tab" value="{{ $active }}" />
                    <div class="col-md-3">
...
@section('javascript')
    <script>
        $('document').ready(function () {
            ...
            $('.panel-actions .voyager-trash').click(function () {
                ...
                $('#delete_form')[0].action = '{{ route('tenant-voyager.settings.delete', [ 'id' => '__id' ]) }}'.replace('__id', $(this).data('id'));
    ...
    <div style="display:none">
        <input type="hidden" id="upload_url" value="{{ route('tenant-voyager.upload') }}">
