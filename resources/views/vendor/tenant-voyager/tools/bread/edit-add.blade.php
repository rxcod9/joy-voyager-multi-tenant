...
<ol class="breadcrumb hidden-xs">
    <li class="active">
        <a href="{{ route('tenant-voyager.dashboard')}}"><i class="voyager-boat"></i> {{ __('voyager::generic.dashboard') }}</a>
    </li>
    <li class="active">
        <a href="{{ route('tenant-voyager.bread.index') }}">
            {{ __('voyager::generic.bread') }}
        </a>
    </li>
    <li class="active">
        @if(isset($dataType->id))
        <a href="{{ route('tenant-voyager.bread.edit', $table) }}">
            {{ $display_name }}
        </a>
        @else
        <a href="{{ route('tenant-voyager.bread.create', $table) }}">
            {{ $display_name }}
        </a>
        @endif
    </li>
    ...
    <div class="page-content container-fluid" id="voyagerBreadEditAdd">
        <div class="row">
            <div class="col-md-12">
                ...
                <form action="@if(isset($dataType->id)){{ route('tenant-voyager.bread.update', $dataType->id) }}@else{{ route('tenant-voyager.bread.store') }}@endif"
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    ...
                                    @if($isModelTranslatable)
                                        @include('tenant-voyager::multilingual.input-hidden', [
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('voyager::bread.icon_hint') }} <a
                                                href="{{ route('tenant-voyager.compass.index') }}#fonts"
                        ...
                        <div class="panel-body">
                                    ...
                                    <div class="col-xs-2">
                                        @if($isModelTranslatable)
                                            @include('tenant-voyager::multilingual.input-hidden', [
                                                ...
                                                '_field_trans' => $dataRow ? get_field_translations($dataRow, 'display_name') : json_encode([config('tenant-voyager.multilingual.default') => ucwords(str_replace('_', ' ', $data['field']))]),
                                            ])
                                        @endif
                                        <input type="text" class="form-control"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('tenant-voyager::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
...
@include('tenant-voyager::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
        ...
        function populateRowsFromTable(dropdown){
            ...
            $.get('{{ route('tenant-voyager.database.index') }}/' + tbl, function(data){
