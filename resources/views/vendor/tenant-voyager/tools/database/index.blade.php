...
@section('page_header')
    <h1 class="page-title">
        ...
        <a href="{{ route('tenant-voyager.database.create') }}" class="btn btn-success"><i class="voyager-plus"></i>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                @foreach($tables as $table)
                    @continue(in_array($table->name, config('tenant-voyager.database.tables.hidden', [])))
                    <tr>
                        <td>
                            <p class="name">
                                <a href="{{ route('tenant-voyager.database.show', $table->prefix.$table->name) }}"
                        ...
                        <td>
                            ...
                            @if($table->dataTypeId)
                                <a href="{{ route('tenant-voyager.' . $table->slug . '.index') }}"
                                   class="btn-sm btn-warning browse_bread">
                                    <i class="voyager-plus"></i> {{ __('voyager::database.browse_bread') }}
                                </a>
                                <a href="{{ route('tenant-voyager.bread.edit', $table->name) }}"
                            ...
                            @else
                                <a href="{{ route('tenant-voyager.bread.create', $table->name) }}"
                        ...
                        <td class="actions">
                            ...
                            <a href="{{ route('tenant-voyager.database.edit', $table->prefix.$table->name) }}"
        ...
        $(function () {
            ...
            $('td.actions').on('click', '.delete_table', function (e) {
                ...
                if ($(this).hasClass('remove-bread-warning')) {
                    ...
                    $('#delete_table_form')[0].action = '{{ route('tenant-voyager.database.destroy', ['database' => '__database']) }}'.replace('__database', table)
            ...
            $('table .bread_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_bread_form')[0].action = '{{ route('tenant-voyager.bread.delete', '__id') }}'.replace('__id', id);
