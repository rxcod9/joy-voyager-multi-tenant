...
@section('page_header')
    <h1 class="page-title">
        ...
        @can('add',app($dataType->model_name))
            <a href="{{ route('tenant-voyager.'.$dataType->slug.'.create') }}" class="btn btn-success">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="no-sort no-click bread-actions">
                                        ...
                                        @endcan
                                        @can('edit', $data)
                                            <a href="{{ route('tenant-voyager.'.$dataType->slug.'.edit', $data->{$data->getKeyName()}) }}" class="btn btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> {{ __('voyager::generic.edit') }}
                                            </a>
                                        @endcan
                                        @can('edit', $data)
                                            <a href="{{ route('tenant-voyager.'.$dataType->slug.'.builder', $data->{$data->getKeyName()}) }}" class="btn btn-sm btn-success pull-right">
...
@section('javascript')
    ...
    <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#delete_form')[0].action = '{{ route('tenant-voyager.'.$dataType->slug.'.destroy', ['id' => '__menu']) }}'.replace('__menu', $(this).data('id'));
