@if(isset($dataTypeContent->{$row->field}))
        ...
        @foreach(json_decode($dataTypeContent->{$row->field}) as $file)
          <div data-field-name="{{ $row->field }}">
            <a class="fileType" target="_blank"
              href="{{ Storage::disk(config('tenant-voyager.storage.disk'))->url($file->download_link) ?: '' }}"
    ...
    @else
      <div data-field-name="{{ $row->field }}">
        <a class="fileType" target="_blank"
          href="{{ Storage::disk(config('tenant-voyager.storage.disk'))->url($dataTypeContent->{$row->field}) }}"
