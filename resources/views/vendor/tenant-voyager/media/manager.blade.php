                ...
                <div class="file_link selected" aria-hidden="true" data-toggle="tooltip" data-placement="auto" :title="file">
                    <div class="link_icon">
                        <template v-if="fileIs(file, 'image')">
                            <div class="img_icon" :style="imgIcon('{{ Storage::disk(config('tenant-voyager.storage.disk'))->url('/') }}'+file)"></div>
                        </template>
    ...
    Vue.component('media-manager', {
        ...
        methods: {
            getFiles: function() {
                var vm = this;
                vm.is_loading = true;
                $.post('{{ route('tenant-voyager.media.files') }}', { folder: vm.current_folder, _token: '{{ csrf_token() }}', details: vm.details }, function(data) {
                    vm.files = [];
            ...
            renameFile: function(object) {
                ...
                $.post('{{ route('tenant-voyager.media.rename') }}', {
            ...
            createFolder: function(e) {
                ...
                $.post('{{ route('tenant-voyager.media.new_folder') }}', { new_folder: vm.current_folder+'/'+name, _token: '{{ csrf_token() }}' }, function(data) {
					if(data.success == true){
            ...
            deleteFiles: function() {
                ...
                var vm = this;
                $.post('{{ route('tenant-voyager.media.delete') }}', {
            ...
            moveFiles: function(e) {
				...
				$.post('{{ route('tenant-voyager.media.move') }}', {
                    path: vm.current_folder,
                    files: vm.selected_files,
            ...
            crop: function(mode) {
				...
				$.post('{{ route('tenant-voyager.media.crop') }}', postData, function(data) {
					if (data.success) {
        ...
        mounted: function() {
            ...
            if (this.allowUpload && !dropzone.hasClass('dz-clickable')) {
                dropzone.dropzone({
                    timeout: 180000,
                    url: '{{ route('tenant-voyager.media.upload') }}',
