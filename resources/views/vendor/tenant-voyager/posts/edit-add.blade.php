...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if($edit){{ route('tenant-voyager.posts.update', $dataTypeContent->id) }}@else{{ route('tenant-voyager.posts.store') }}@endif" method="POST" enctype="multipart/form-data">
                            ...
                            @foreach($dataTypeRows as $row)
                                @if(!in_array($row->field, $exclude))
                                    ...
                                    @if (isset($row->details->formfields_custom))
                                        @include('tenant-voyager::formfields.custom.' . $row->details->formfields_custom)
                                    @else
                                        <div class="form-group @if($row->type == 'hidden') hidden @endif @if(isset($display_options->width)){{ 'col-md-' . $display_options->width }}@endif" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                            {{ $row->slugify }}
                                            <label for="name">{{ $row->getTranslatedAttribute('display_name') }}</label>
                                            @include('tenant-voyager::multilingual.input-hidden-bread-edit-add')
                                            @if($row->type == 'relationship')
                                                @include('tenant-voyager::formfields.relationship', ['options' => $row->details])
                                            @else
                                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                                            @endif
    ...
    <script>
        ...
        $('document').ready(function () {
            ...
            $('#confirm_delete').on('click', function(){
                $.post('{{ route('tenant-voyager.'.$dataType->slug.'.media.remove') }}', params, function (response) {
                    if ( response
                        && response.data
