...
class VoyagerSettingsController extends Controller
{
    ...
    {
        // Check permission
        $this->authorize('browse', TenantVoyager::model('Setting'));

        $data = TenantVoyager::model('Setting')->orderBy('order', 'ASC')->get();
        ...
        $groups_data = TenantVoyager::model('Setting')->select('group')->distinct()->get();
        $groups = [];
        ...
        return TenantVoyager::view('tenant-voyager::settings.index', compact('settings', 'groups', 'active'));
    }

    public function store(Request $request)
    {
        // Check permission
        $this->authorize('add', TenantVoyager::model('Setting'));

        $key = implode('.', [Str::slug($request->input('group')), $request->input('key')]);
        $key_check = TenantVoyager::model('Setting')->where('key', $key)->get()->count();
        ...
        $lastSetting = TenantVoyager::model('Setting')->orderBy('order', 'DESC')->first();
        ...
        TenantVoyager::model('Setting')->create($request->except('setting_tab'));
    ...
    {
        // Check permission
        $this->authorize('edit', TenantVoyager::model('Setting'));

        $settings = TenantVoyager::model('Setting')->all();
    ...
    {
        // Check permission
        $this->authorize('delete', TenantVoyager::model('Setting'));

        $setting = TenantVoyager::model('Setting')->find($id);

        TenantVoyager::model('Setting')->destroy($id);
    ...
    public function move_up($id)
    {
        ...
        $previousSetting = TenantVoyager::model('Setting')
    ...
    public function delete_value($id)
    {
        ...
        if (isset($setting->id)) {
            ...
            if ($setting->type == 'image') {
                if (Storage::disk(config('tenant-voyager.storage.disk'))->exists($setting->value)) {
                    Storage::disk(config('tenant-voyager.storage.disk'))->delete($setting->value);
                }
            }
            $setting->value = '';
            $setting->save();
