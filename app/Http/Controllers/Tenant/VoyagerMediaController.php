...
class VoyagerMediaController extends Controller
{
    ...
    {
        $this->filesystem = config('tenant-voyager.storage.disk');
    }
    ...
    public function index()
    {
        ...
        return TenantVoyager::view('tenant-voyager::media.index');
    }
    ...
    public function files(Request $request)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('tenant-voyager.hidden_files')) {
                    continue;
                }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('tenant-voyager.media.allowed_mimetypes', '*');
