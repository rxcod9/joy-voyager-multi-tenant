...
class VoyagerBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $dataType = TenantVoyager::model('DataType')->where('slug', '=', $slug)->first();
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (TenantVoyager::actions() as $action) {
        ...
        $view = 'tenant-voyager::bread.browse';

        if (view()->exists("tenant-voyager::$slug.browse")) {
            $view = "tenant-voyager::$slug.browse";
        }
        // dd($view);

        return TenantVoyager::view($view, compact(
            'actions',
            'dataType',
            'dataTypeContent',
            'isModelTranslatable',
    ...
    public function show(Request $request, $id)
    {
        ...
        $view = 'tenant-voyager::bread.read';

        if (view()->exists("tenant-voyager::$slug.read")) {
            $view = "tenant-voyager::$slug.read";
        }

        return TenantVoyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $view = 'tenant-voyager::bread.edit-add';

        if (view()->exists("tenant-voyager::$slug.edit-add")) {
            $view = "tenant-voyager::$slug.edit-add";
        }

        return TenantVoyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            $redirect = redirect()->route("tenant-voyager.{$dataType->slug}.index");
        } else {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        return redirect()->route("tenant-voyager.{$dataType->slug}.index")->with($data);
    }
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if ($image != config('tenant-voyager.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
    ...
    public function order(Request $request)
    {
        ...
        if (empty($dataType->order_column) || empty($dataType->order_display_column)) {
            return redirect()
            ->route("tenant-voyager.{$dataType->slug}.index")
            ->with([
        ...
        $dataRow = TenantVoyager::model('DataRow')->whereDataTypeId($dataType->id)->whereField($display_column)->first();

        $view = 'tenant-voyager::bread.order';

        if (view()->exists("tenant-voyager::$slug.order")) {
            $view = "tenant-voyager::$slug.order";
        }
