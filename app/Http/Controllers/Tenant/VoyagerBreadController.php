...
class VoyagerBreadController extends Controller
{
    public function index()
    {
        ...
        $dataTypes = TenantVoyager::model('DataType')->select('id', 'name', 'slug')->get()->keyBy('name')->toArray();
        ...
        return TenantVoyager::view('tenant-voyager::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = TenantVoyager::model('DataType')->whereName($table)->first();
        ...
        return TenantVoyager::view('tenant-voyager::tools.bread.edit-add', $data);
    }

    private function prepopulateBreadInfo($table)
    {
        $displayName = Str::singular(implode(' ', explode('_', Str::title($table))));
        $modelNamespace = config('tenant-voyager.models.namespace', app()->getNamespace());
    ...
    public function store(Request $request)
    {
        ...
        try {
            $dataType = TenantVoyager::model('DataType');
            ...
            return redirect()->route('tenant-voyager.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('tenant-voyager.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = TenantVoyager::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return TenantVoyager::view('tenant-voyager::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $dataType = TenantVoyager::model('DataType')->find($id);
    ...
    public function destroy($id)
    {
        ...
        $res = TenantVoyager::model('DataType')->destroy($id);
        $data = $res
        ...
        if (!is_null($dataType)) {
            TenantVoyager::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = TenantVoyager::modelClass('DataRow');
            ...
            $newRow->order = intval(TenantVoyager::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = TenantVoyager::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = TenantVoyager::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
    ...
    {
        TenantVoyager::model('DataRow')->destroy($id);
