...
class VoyagerMenuController extends Controller
{
    ...
    {
        $menu = TenantVoyager::model('Menu')->findOrFail($id);

        $this->authorize('edit', $menu);

        $isModelTranslatable = is_bread_translatable(TenantVoyager::model('MenuItem'));

        return TenantVoyager::view('tenant-voyager::menus.builder', compact('menu', 'isModelTranslatable'));
    }

    public function delete_menu($menu, $id)
    {
        $item = TenantVoyager::model('MenuItem')->findOrFail($id);
        ...
        return redirect()
            ->route('tenant-voyager.menus.builder', [$menu])
            ->with([
    ...
    public function add_item(Request $request)
    {
        ...
        unset($data['id']);
        $data['order'] = TenantVoyager::model('MenuItem')->highestOrderMenuItem();

        // Check if is translatable
        $_isTranslatable = is_bread_translatable(TenantVoyager::model('MenuItem'));
        if ($_isTranslatable) {
            // Prepare data before saving the menu
            $trans = $this->prepareMenuTranslations($data);
        }

        $menuItem = TenantVoyager::model('MenuItem')->create($data);
        ...
        return redirect()
            ->route('tenant-voyager.menus.builder', [$data['menu_id']])
            ->with([
    ...
    public function update_item(Request $request)
    {
        ...
        $menuItem = TenantVoyager::model('MenuItem')->findOrFail($id);
        ...
        return redirect()
            ->route('tenant-voyager.menus.builder', [$menuItem->menu_id])
            ->with([
    ...
    private function orderMenu(array $menuItems, $parentId)
    {
        foreach ($menuItems as $index => $menuItem) {
            $item = TenantVoyager::model('MenuItem')->findOrFail($menuItem->id);
            $item->order = $index + 1;
    ...
    protected function prepareMenuTranslations(&$data)
    {
        ...
        $data['title'] = $trans[config('tenant-voyager.multilingual.default', 'en')];
