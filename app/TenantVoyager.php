...
class TenantVoyager extends Voyager
{
    protected $version;
    ...
    {
        $widgetClasses = config('tenant-voyager.dashboard.widgets');
        $dimmerGroups = [];
        $dimmerCount = 0;
    ...
    {
        $globalCache = config('tenant-voyager.settings.cache', false);
    ...
    {
        if (!empty($file)) {
            return str_replace('\\', '/', Storage::disk(config('tenant-voyager.storage.disk'))->url($file));
        }
    ...
    {
        require __DIR__.'/../routes/tenant-voyager.php';
    }
    ...
    {
        if (!config('tenant-voyager.multilingual.enabled')) {
            return false;
        }
